...
def remove_duplicates_set(s):
    ...
    Parameters:
    - s (string): The string in which duplicated chars will get removed.
...
def gem_counting(stones, gems):
    """
    This function calculates the number of stones that are actually gems.

    E.g.,
    >>> gem_counting("abDFMdm", "af")
    1
    ...
    gem_set = set(gems)
    common_stones = stone_set & gem_set
    return len(common_stones)
...
def students_id(ids):
    """
    Calculates the number of different students who use the shuttle bus.
    ...
    Returns:
    int: The number of different students who use the bus.
    """
    different_students = set(ids)
    return len(different_students)
...
def students_id_occurrences(ids):
    """
    Counts the occurrences of different students who use the shuttle bus.
    ...
    Returns:
    dict: A dictionary containing student IDs as it keys and their occurrences as its values.
    """
    id_dict = {}
    for id in ids:
        if id in id_dict:
...
def word_frequency(paragraph):
    ...
    Parameters:
    paragraph (str): A string that is the given paragraph.

    Returns:
    dict: A dictionary containing the frequency of each word in paragraph.
